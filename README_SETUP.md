...
- ✅ Generate test sample and wait for your confirmation
- ✅ Ask for training parameters (samples, steps, false activation penalty) with suggested values
- ✅ Generate all positive samples using TTS
- ✅ Generate negative samples (adversarial)
- ✅ Augment samples with background noise and reverb
...
- False activation penalty: Default 1.0 (0.5-5.0 range)
...
1. **File Trimming Skip**: Disabled on Windows due to file locking issues
2. **Multiprocessing Disabled**: DataLoader uses `num_workers=0` to avoid pickling errors
3. **Unicode Handling**: ASCII filtering for console output compatibility
